...
function ListExampleFive() {
  ...
  const setupDropTarget = (containerId: string) => {
    return dropTargetForElements({
      ...
      onDrag({ self }) {
        ...
        if (targetData && targetData.itemId) {
          const edge = extractClosestEdge(targetData);
          setHighlight({ id: targetData.itemId, edge });
        }
      },
      onDrop({ source, location }) {
        ...
        if (!sourceId || !target || !target.itemId || !target.containerId) {
          setHighlight(null);
          return;
        }
        ...
        if (sourceItems === targetItems && startIndex === finishIndex) {
          setHighlight(null);
          return;
        }
  ...
  return (
    <div style={{ padding: '20px' }}>
      <h1>List Example Five - Drag and Drop with Highlight</h1>
...
export default ListExampleFive;
